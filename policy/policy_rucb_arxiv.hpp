...
//Relative Upper Confidence Bound (RUCB), arXiv version
class RUCBArxivPolicy : public Policy{
  const uint K;
  const uint T;
  const double alpha;
...
public:
  RUCBArxivPolicy(uint K, uint T, double alpha = 0.51): K(K), T(T), alpha(alpha) {
